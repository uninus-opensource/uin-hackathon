import { FC, MouseEventHandler, ReactElement, ReactNode, useState } from "react";
    ...
    <div className="flex justify-start gap-x-2">
      ...
      {Array.from({ length: Math.min(maxButtons, totalPage) }, (_, i) => (
        <Button
          ...
          size="sm"
        >
          <span className={startPage + i === currentPage ? "text-white" : "text-grey"}>
            {startPage + i}
          </span>
        </Button>
      ))}
      ...
      <Button
        onClick={() =>
          Number(page) < Number(meta?.meta?.totalPage) && setPage(Number(page) + 1)
        }
        variantType="outline"
        size="sm"
      >
            ...
            {table.getRowModel().rows.map((row) => (
              <tr key={row.id} className="even:bg-grey-100 odd:bg-grey-50">
                {row.getVisibleCells().map((cell, index) => (
                  <td key={index} className="p-4 text-grey-600 font-normal text-sm">
